...
monthly observations over two or three years, rather than four snapshots, would increase the effective sample size and stabilize the CV results. deflating the retail trade target by regional CPI would let the model target real consumption growth. regional fixed effects would control for structural differences between oblasts. comparing this linear specification against Random Forest or Ridge regression would test how much the linearity assumption costs in accuracy.
